  ...
  compute_group = \(
    ...
    if (any(is_bad)) {
      if (na.rm) {
        ...
        cli::cli_warn(
          "Group contains missing or non-finite values. \\
           Set {.arg na.rm} = {.val TRUE} to drop them silently."
        )
    ...
    if (!is.null(n_harmonics) && n_harmonics > max_k) {
      cli::cli_warn(
        "{.arg n_harmonics} ({n_harmonics}) exceeds the Nyquist limit for \\
         this group ({max_k}). Using {max_k}."
      )
    }
    ...
    calc_fourier_vec <- function(t_vec) {
      ...
      if (anyNA(imag_part)) {
        ...
        if (max_imag > 1e-8) {
          cli::cli_warn(
            "Unexpectedly large imaginary residual ({signif(max_imag, 3)}). \\
             Results may be unreliable. \\
             Check for irregular {.arg x} spacing or unresolved duplicates."
          )
        }
      }
    ...
    # Re-add the trend on the fine grid, if there was one calculated
