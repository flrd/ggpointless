...
test_that("geom_ridgeline_fade [doc ex 1]: basic ridgeline", {
  ...
  # ggridges' geom_density_ridges auto-computes density; our drop-in is
  # `geom_ridgeline_density_fade()` (the convenience wrapper that pairs
  # `geom_ridgeline_fade` with `stat_density`). Raw `geom_ridgeline_fade`
  # requires an explicit `height` aesthetic — iris has no such column.
  expect_no_error(ggplotGrob(
    ggplot(iris, aes(x = Sepal.Length, y = Species, fill = Species)) +
      geom_ridgeline_density_fade(alpha = 0.7)
  ))
})
...
test_that("path_fade: alpha_mode rendering tier (compositing vs viewport mask)", {
  # Locks in the structural invariant that explains the RStudio
  # "Unknown group" warning differential documented in ?geom_path_fade:
  #   - step mode emits GridGroup objects (groupGrob/dest.in compositing).
  #     Their device-allocated group IDs become stale on RStudio plot pane
  #     replay, which is what surfaces "Unknown group, N" warnings.
  #   - gradient mode wraps each path group in viewport(mask = polylineGrob).
  #     There is no group reference to invalidate, so no warning fires.
  # Headless reproduction of the warning itself is not feasible (the replay
  # path is RStudio-specific), so this test asserts the upstream cause
  # rather than the symptom.
  set.seed(42)
  d <- data.frame(x = 1:30, y = cumsum(rnorm(30)))
  ...
  # Open a device for the whole test so dev.capabilities() and the
  # makeContent calls all see the same (capable) backend.
  grDevices::png(tempfile(), width = 400, height = 300)
  on.exit(grDevices::dev.off(), add = TRUE)

  caps <- grDevices::dev.capabilities()
  skip_if_not(
    "dest.in" %in% caps[["compositing"]] && isTRUE(caps[["clippingPaths"]]),
    "test device lacks compositing or clipping path support"
  )

  rendered_children <- function(mode) {
    p <- ggplot(d, aes(x, y)) +
      geom_line_fade(linewidth = 1, alpha_mode = mode)
    pfg <- find_pfg(p)
    grid::grid.newpage()
    grid::pushViewport(
      grid::viewport(xscale = c(0, 30), yscale = range(d$y))
    )
    grid::makeContent(pfg)$children
  }

  step_children <- rendered_children("step")
  expect_true(
    any(vapply(step_children, inherits, logical(1), "GridGroup")),
    info = "step mode should emit at least one groupGrob (compositing tier)"
  )

  grad_children <- rendered_children("gradient")
  expect_false(
    any(vapply(grad_children, inherits, logical(1), "GridGroup")),
    info = "gradient mode should not emit groupGrob (uses viewport mask tier)"
  )
  expect_true(
    any(vapply(grad_children, function(g) {
      !is.null(g$vp) && inherits(g$vp$mask, "GridMask")
    }, logical(1))),
    info = "gradient mode should wrap each group in viewport(mask = ...)"
  )
})
