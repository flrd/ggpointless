...
- [`geom_arch()`](https://flrd.github.io/ggpointless/reference/geom_catenary.md)
  ...
  – draws a catenary arch
- [`geom_catenary()`](https://flrd.github.io/ggpointless/reference/geom_catenary.md)
  &
  [`stat_catenary()`](https://flrd.github.io/ggpointless/reference/geom_catenary.md)
...
- [`geom_chaikin()`](https://flrd.github.io/ggpointless/reference/geom_chaikin.md)
  ...
  – smooths paths using Chaikin’s corner cutting algorithm
- [`geom_fourier()`](https://flrd.github.io/ggpointless/reference/geom_fourier.md)
  &
  [`stat_fourier()`](https://flrd.github.io/ggpointless/reference/geom_fourier.md)
  – fits a Fourier series to `x`/`y` observations
- [`geom_gridline()`](https://flrd.github.io/ggpointless/reference/geom_gridline.md)
  – gridlines as a layer (drawn above other geoms)
...
- [`geom_pointless()`](https://flrd.github.io/ggpointless/reference/geom_pointless.md)
  ...
  – emphasises selected observations with points
...
- [`geom_abline_fade()`](https://flrd.github.io/ggpointless/reference/geom_abline_fade.md)
  /
  [`geom_hline_fade()`](https://flrd.github.io/ggpointless/reference/geom_abline_fade.md)
  /
  [`geom_vline_fade()`](https://flrd.github.io/ggpointless/reference/geom_abline_fade.md)
...
- [`geom_col_fade()`](https://flrd.github.io/ggpointless/reference/geom_col_fade.md)
  /
  [`geom_bar_fade()`](https://flrd.github.io/ggpointless/reference/geom_col_fade.md)
- [`geom_density_fade()`](https://flrd.github.io/ggpointless/reference/geom_area_fade.md)
- [`geom_freqpoly_fade()`](https://flrd.github.io/ggpointless/reference/geom_area_fade.md)
...
- [`geom_path_fade()`](https://flrd.github.io/ggpointless/reference/geom_path_fade.md)
  ...
  /
  [`geom_step_fade()`](https://flrd.github.io/ggpointless/reference/geom_path_fade.md)
- [`geom_point_glow()`](https://flrd.github.io/ggpointless/reference/geom_point_glow.md)
- [`geom_rect_fade()`](https://flrd.github.io/ggpointless/reference/geom_rect_fade.md)
- [`geom_ridgeline_fade()`](https://flrd.github.io/ggpointless/reference/geom_ridgeline_fade.md)
  /
  [`geom_ridgeline_density_fade()`](https://flrd.github.io/ggpointless/reference/geom_ridgeline_fade.md)
  /
  [`geom_ridgeline_freqpoly_fade()`](https://flrd.github.io/ggpointless/reference/geom_ridgeline_fade.md)
  /
  [`geom_ridgeline_histogram_fade()`](https://flrd.github.io/ggpointless/reference/geom_ridgeline_fade.md)
...
![Four points connected by a catenary curve sagging between adjacent
points.](reference/figures/README-geom-catenary-1.png)
...
![Same four-point catenary with per-segment overrides — the first
segment sags deeper, the second tighter, the third uses a longer chain
length.](reference/figures/README-geom-catenary-sag-1.png)
...
![Inverted catenary arches connecting four points; each arch peaks
midway between two adjacent
points.](reference/figures/README-geom-arch-height-sag-1.png)
...
![Four polygon shapes drawn with sharp dashed outlines and overlaid with
smoothed Chaikin corner-cut
curves.](reference/figures/README-geom-chaikin-1.png)
...
![Noisy point cloud with a single Fourier-series fit overlaid; the curve
follows the upward drift while smoothing the cyclic
component.](reference/figures/README-geom-fourier-1.png)
...
![Noisy point cloud with two overlaid Fourier-series fits: one without
detrending shows the linear drift, one with linear detrending isolates
the cyclic
component.](reference/figures/README-geom-fourier-detrending-1.png)
...
![Vertical bar chart of mpg car-class counts with horizontal grid lines
drawn on top of the bars.](reference/figures/README-geom-gridline-1.png)
...
![Horizontal bar chart of mpg car-class counts with vertical white grid
lines drawn on top of the
bars.](reference/figures/README-geom-gridline-horizontal-1.png)
...
![Same horizontal bar chart on a log10 x scale; grid line positions
follow the transformed
breaks.](reference/figures/README-geom-gridline-horizontal-log10-1.png)
...
![Same horizontal bar chart with bolder white grid lines drawn via an
explicit colour and linewidth
override.](reference/figures/README-geom-gridline-horizontal-theme-dark-1.png)
...
![Lexis diagram showing five 45-degree lifelines coloured by category,
with dotted gap-fillers bridging gaps between events of the same
cohort.](reference/figures/README-geom-lexis-1.png)
...
![Yearly sunspot data from 1977 to 1986 as a step
chart.](reference/figures/README-geom-pointless-1.png)
...
![Area chart of a 60-step random walk, fill fading from opaque at the
line to transparent at the y=0
baseline.](reference/figures/README-geom-area-fade-1.png)
...
![Frequency polygon of iris Sepal.Length filled below the line with the
gradient reversed — transparent at the line, opaque at the y=0
baseline.](reference/figures/README-geom-freqpoly-fade-1.png)
...
![Twelve overlapping monthly density curves of Nottingham temperatures,
fill mapped to temperature so colour shifts from indigo (cold) through
pink to amber (warm); each fades to transparent at the
baseline.](reference/figures/README-geom-density-fade-1.png)
...
![Twelve ridgeline density curves stacked from January at the bottom to
December at the top, fill mapped to temperature, fading to transparent
at each row's
baseline.](reference/figures/README-geom-ridgeline-density-fade-2D-1.png)
...
![Same twelve monthly ridgelines drawn as binned frequency polygons
(step-shaped) instead of smooth
densities.](reference/figures/README-geom-ridgeline-freqpoly-fade-2D-1.png)
...
![Same twelve monthly ridgelines drawn as 40-bin histograms — each row
is a stack of fading bars instead of a smooth
curve.](reference/figures/README-geom-ridgeline-histogram-fade-2D-1.png)
...
![Anscombe's quartet rendered as four scatter facets where each point
carries a soft purple radial
glow.](reference/figures/README-geom-point-glow-1.png)
...
![mtcars weight-vs-mpg scatter with two fading arrows between the same
two points — one curved, one straight — colour-labelled by
geom.](reference/figures/README-geom-segment-fade-1.png)
...
![A stylised fish (ichthys) outline drawn as a single path with the line
fading to transparent at both the start and the
end.](reference/figures/README-geom-path-fade-1.png)
...
![Three coloured reference lines on a unit panel — diagonal (purple),
horizontal (pink), and vertical (amber) — each fading to transparent
along its length.](reference/figures/README-geom-abline-fade-1.png)
...
![The same three reference lines drawn under coord_polar, becoming
concentric arcs and rays with the fade preserved along each
line.](reference/figures/README-geom-abline-fade-polar-coord-1.png)
...
![Standard normal density curve filled as a fading area beneath the
curve, with the outline drawn as a separate line that fades to
transparent at both
tails.](reference/figures/README-stat-function-1.png)
...
![Step chart of US unemployment for the first 25 economics observations,
the line fading from opaque on the right to nearly transparent on the
left.](reference/figures/README-geom-step-fade-1.png)
...
![Same unemployment step chart with a vertical band from July 1968 to
July 1969 shaded by a rectangle that fades to transparent at its top
edge.](reference/figures/README-geom-rect-fade-1.png)
...
![Vertical bar chart of mpg class counts with each bar filled by an
alpha gradient that fades from opaque at the top to transparent at the
baseline; bars have rounded
corners.](reference/figures/README-geom-bar-fade-1.png)
...
![Vertical bar chart of BOD demand over time with rounded corners and a
global alpha gradient — fade depth scales with each bar's height
relative to the tallest.](reference/figures/README-geom-col-fade-1.png)
...
![Histogram of Old Faithful eruption durations (30 bins) with each bar
filled by an alpha gradient; gradient strength is shared across bars
(global scope).](reference/figures/README-geom-histogram-fade-1.png)
...
![Old Faithful eruption-duration histogram drawn as a stack of unit
cells per bar, with cell fill running from indigo blue for short
eruptions to amber for long
ones.](reference/figures/README-geom-unit-histogram-faithful-1.png)
...
![Horizontal isotype bar chart counting diamonds per cut; each cell
represents one hundred
observation.](reference/figures/README-geom-unit-bar-1.png)
...
or Okabe–Ito; and raise `alpha_fade_to` when the faded region carries
data your readers need to find.
...
information rather than ordering, prefer
...
The 2D gradients used by
[`geom_area_fade()`](https://flrd.github.io/ggpointless/reference/geom_area_fade.md)
and friends depend on **Porter-Duff compositing**[^1], a feature of R’s
graphics engine added in R 4.2. When the active device does not support
compositing
(e.g. [`grDevices::pdf()`](https://rdrr.io/r/grDevices/pdf.html)), the
fade family falls back to a single-colour vertical fade — the horizontal
colour gradient is lost, only the vertical alpha fade survives, and a
one-time message is emitted:
...
on Linux and macOS — *do* support compositing, so the 2D gradient works
out of the box. In RStudio, go to *Tools \> Global Options \> Graphics
