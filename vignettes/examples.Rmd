...
We'll start to recreate Gregor Aisch's plot of [carbon dioxide concentration over time](https://blog.datawrapper.de/weekly-chart-carbon-dioxide/). `co2_ml` is a data.frame of monthly CO~2~ measurements taken at Mauna Loa, Hawaii (see also `co2` from the `datasets` package). 
...
```{r co2_plot, fig.height=6, echo=FALSE, warning=FALSE}
p
```
...
The following two charts show age-adjusted daily cases and death rates for vaccinated and unvaccinated people in 26 states in the USA. This graphic is inspired by the New York Times' [Coronavirus Map and Cases](https://www.nytimes.com/interactive/2021/us/covid-cases.html). The source of the second data set is [Centers for Disease Control and Prevention](https://covid.cdc.gov/covid-data-tracker/#rates-by-vaccine-status). 

```{r}
data(covid_vac)
```

Again, we first create a helper function to label the facet labels: 
...
p <- p + scale_x_date(expand = expansion(mult = c(0, 0.2))) +
  ...
  scale_alpha_manual(values = c(unvaccinated = 1, 'fully vaccinated' = 0.5)) +
  scale_color_manual(values = c(case = "#f9491b", death = "#000000"))
...
p <- p + theme(panel.grid.major.y = element_line(linetype = "12"))
