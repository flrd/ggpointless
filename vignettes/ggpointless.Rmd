...
## Plotting live lines with geom_lexis
...
If there is a gap in an event, this is shown by a horizontal line, which can be hidden by `gap_filler = FALSE`. Also, `geom_lexis()` creates an additional variable, `type` that takes two values: "solid" and "dotted". You can map the `type` the linetype aesthetic (or any other aesthetic) using `after_stat()`, see below. Since the values of the `type` column have no contextual meaning, they are probably best used in conjunction with `scale_linetype_identity()` to not show a legend.
