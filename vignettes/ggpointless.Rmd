...
`geom_catenary()` calculates a default value for the lnegth of the chain that is set
to 2 times the Euclidean distance of all x/y coordinates. You can change this behaviour and
the value of `chainLength` will be applied to each 'chain link':

```{r geom-catenary-chainLength, fig.show='hold'}
ggplot(data.frame(x = c(0, 1, 3), y = c(1, 1, 1)), aes(x, y)) +
...
If you want to draw a chain where each piece has a different value (or another attribute) for `chainLength` you can add a list to an ggplot object:
...
ggplot() + Map(
  function(x, y, chainLength, colour) {
    geom_catenary(aes(x, y),
                  data.frame(x = x + 0:1, y = y + 0:1),
                  chainLength = chainLength,
                  col = colour)
  },
  x = lst[["x"]],
  y = lst[["y"]],
  chainLength = lst[["chainLength"]],
  colour = lst[["colour"]]
)
```
