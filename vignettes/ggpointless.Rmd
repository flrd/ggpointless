...
If there is a gap in an event, by default, a horizontal line is drawn, which can be hidden setting `gap_filler = FALSE`. If you want to make a visual distinction between the ascending lines and the connecting lines, you can use the variable `type` for this. This is created by `geom_lexis()` and takes two values: "solid" and "dotted". With `after_stat()` you can map the linetype (or any other aesthetic) to the variable `type`, see below. 

```{r after_stat, fig.show='hold'}
...
For all arguments that are supported see `?geom_lexis`.



## Data

In addition, the `ggpointless` package contains the following data sets:
