...
The idea of the `geom_lexis()` function is to draw a lifeline for an event from it's start to it's end. The required aesthetics are `x` and `xend`. Here is a first example:
...
You see the coordinates on the vertical y-axis show the difference between `x` and `xend` aesthetics. Also, if there is a gap in an event a horizontal line is drawn, which can be hidden setting `gap_filler = FALSE`.

```{r gap_filler, fig.show='hold'}
p + geom_lexis(gap_filler = FALSE)
```

You can further style the appearance of your plot using the additional arguments. If you e.g. want to make a visual distinction between the ascending lines and the connecting lines, use the variable `type` for this which is created by `geom_lexis()` -- it takes two values: "solid" and "dotted". Use `after_stat()` to map `type` to the linetype aesthetic (or any other aesthetic) to the variable `type`. Also, you might also want to call `scale_linettype_identity`, see below. 
...
The "magic" of `geom_lexis()` happens in `stat_lexis()` when the input data is transformed and the calculations are performed. Part of this is the subtraction of `x` values from the respective `xend` values to get the coordinates on the y-scale, see the next example:
...
```{r dates}
# days
fun <- function(i) as.Date(paste0(i, "-01-01"))
...
In order to change the labels of the y-scale in the prvious plot to, say, years, we make the assumption that 1 year has 365.25 days.
...
p2 <- p1 +
  ...
  labs(subtitle = "Assumption: 365.25 days/year",
       y = "years")
p2
```
