...
p + geom_area_fade()
```
By default, the gradient fully transparent at at `y = 0` (the baseline) and
...
You can control the alpha value the fill fades to using the `alpha_fade_to` 
argument. By that logic you can effectively reverse the direction of the 
gradient. The outline colour is unaffected from the alpha logic.
