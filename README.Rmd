...
You can control the alpha value the fill fades to using `alpha_fade_to` argument.
Below, instead of fading from full opacity towards full transparency at `y = 0` 
alpha starts at 50% and fades to 10%:
...
`geom_area_fade()` supports the `orientation` argument you know and love from other
`geom_*`s. With `orientation = "y"`, you create a horizontal area chart where 
the gradient fades from `x = 0` toward the data values. You can change the
outline colour too.
