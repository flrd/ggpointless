...
### geom_lexis
`geom_lexis()` is a combination of a segment and a point layer. Given a start value and an end value, this functions draws a 45° line which indicates the duration of an event. The geom requires an `x` and and an `xend` aesthetic and calculated the `y` and `yend` coordinates for you.

```{r geom_lexis, warning=FALSE}
df2 <- data.frame(
  key = c("A", "B", "B", "C", "D"),
  x = c(0, 1, 6, 5, 6),
  xend = c(5, 4, 10, 8, 10)
