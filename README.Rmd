...
Add a minimal accent to your plots. `ggpointless` is an extension 
of the [`ggplot2`](https://ggplot2.tidyverse.org/) library making it easy to add a simple point layer to
highlight first, last, minima or maxima observations with the goal to provide
some additional context. Or just some visual sugar.
...
There are two functions in this small package package: `geom_pointless()`, which is powered by `stat_pointless()`. These functions behave like `geom_point()` does, with the addition of a `location` argument. You can set it to `"first"`, `"last"` (default), `"minimum"`, `"maximum"`, and `"all"`, where `"all"` is just shorthand to select `"first"`, `"last"`, `"minimum"` and `"maximum"`.
