...
## geom_area_fade
`geom_area_fade()` behaves like [`geom_area()`](https://ggplot2.tidyverse.org/reference/geom_ribbon.html)
but fills each area with a vertical [linear gradient](https://search.r-project.org/R/refmans/grid/html/patterns.html). 
...
p + geom_area_fade()
```
By default, the gradient is fully transparent at `y = 0` (the baseline) and
proportionally opaque at the data values. That is, opacity scales with absolute
distance from zero, so e.g. `y = -1` and `y = +1` always receive the same alpha. 
The outline colour is unaffected by the alpha logic.

You can control the alpha value the fill fades to using the `alpha` and `alpha_fade_to` 
arguments. You can effectively reverse the direction of the gradient like so:
...
> **Note** -- Not all graphic devices support this kind of gradient, see [`vignette("ggpointless")`](https://flrd.github.io/ggpointless/articles/ggpointless.html) for more details and examples.
...
`position = "stack"` but `position = "identity"` instead, this can make smaller
...
## geom_arch
`geom_arch()` draws a [catenary arch](https://en.wikipedia.org/wiki/Catenary_arch) (inverted catenary curve) between successive points. 

```{r geom-arch-basic-example, warning=FALSE}
...
By default the arch length is twice the Euclidean distance. You can change that
for each segment using the arguments `arch_length` or `arch_height` (vertical
rise above the _highest_ endpoint of each segment). 

```{r geom-arch-height-sag, warning=FALSE}
df_arch <- data.frame(x = seq_len(4), y = c(1, 1, 0, 2))
...
## geom_catenary
`geom_catenary()` draws a [catenary curve](https://en.wikipedia.org/wiki/Catenary_arch),
i.e., a flexible curve that simulates a chain or rope hanging loosely between
successive points. By default, the chain length is twice the Euclidean distance
between each `x`/`y` pair. The shape can be controlled via `chain_length` or 
`sag`, i.e, vertical drop below the _lowest_ endpoint of each segment.
...
smallest value of each segment. `NA` keeps the default. If you provide `sag` and
`chain_length` for the same segment, then `sag` wins.
...
## geom_fourier
`geom_fourier()` fits a Fourier series (via [fft()](https://search.r-project.org/R/refmans/stats/html/fft.html)) to the supplied `x`/`y` observations and renders the reconstructed smooth curve. By default all harmonics up to the Nyquist limit are used, giving an exact interpolating fit; reducing `n_harmonics` progressively smooths the result. 

The animation below shows how `geom_fourier()` approximates a square wave as
the number of harmonics grows from `n = 1` to the Nyquist limit, where the
[Gibbs phenomenon](https://en.wikipedia.org/wiki/Gibbs_phenomenon) is visible.
